...
public class DigitalSignature {
	/**
	 * 通过请求参数生成 密钥 md5 加密的前16位 就是 AES-128-CBC密匙
	 * 
	...
	/**
	 * 数字加密方法 前端同等实现 java那边使用AES
	 * 128位填充模式：AES/CBC/PKCS7Padding加密方法，在nodejs中采用对应的aes-128-cbc加密方法就能对应上，因为有使用向量（iv），所以nodejs中要用createCipheriv方法，而不是createCipher。
	...
	/**
	 * 解密数字凭证获取 token _timestamp _username 先 BASE64 解密，
	 * 再使用AES-128-CBC对称加密算法解密(128位AES/ECB/PKCS7Padding加密/解密) AES密匙： getMethod是get密匙：
	 * 整条 _url参数 request.getQueryString 就是问号后面的值 getMethod是post密匙： _json body字符串；（所以
	 * POST请求必须是JSON格式，校验到非JSON格式的可以直接拒绝请求
	 * 
	 * @param reqParam                      请求参数 不管是get请求还是post请求，一律转为_utf-8字符串
